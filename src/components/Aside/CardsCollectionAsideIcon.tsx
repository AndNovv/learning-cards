...
const CardsCollectionAsideIcon = ({ collection }: { collection: WordCollection }) => {
    ...
    return (
        <div onClick={() => router.push(`/collection/${collection.id}`)} className='group relative flex items-center justify-center w-14 h-14 rounded-full hover:rounded-3xl bg-accent hover:bg-primary hover:text-primary-foreground cursor-pointer transition-all'>
            <Scroll />
